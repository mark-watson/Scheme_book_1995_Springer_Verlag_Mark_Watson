...
(define (make-player-move game)
  ...
  (let* ((response (string-capitalize (symbol->string (read))))
	 ...
	 (row (- (char->digit (string-ref response 1)) 1)))
...
(define (best-computer-move game)
...
;;
 ;  The following function updates the move values data structures.
 ;  This version does nothing, simply using the Joseki data.  This
 ;  function should be redefined in strategy and tactical File Modules.
 ;;

(define (update-move-values game)
#f)
